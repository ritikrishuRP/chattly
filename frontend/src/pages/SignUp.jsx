...
function SignUp() {
  ...
  return (
    <div className='w-full h-screen bg-slate-200 flex items-center justify-center'>
        ...
        <form className='w-full flex flex-col gap-5 items-center justify-center' onSubmit={handleSignUp}>

          <input type='text' placeholder='username' className='w-[90%] h-[50px] outline-none
          ...
          hover:shadow-inner cursor-pointer' disabled={loading}>{loading?"Loading...":"Sign Up"}</button>
          <p className='cursor-pointer' onClick={()=>navigate("/login")}>Already have An Account<span className='text-[#20c7ff] text-bold cursor-pointer'>Login</span></p>
        </form>
